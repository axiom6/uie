...
class Radar
  ...
  constructor:( @g, @isRadar=true, @width=1200, @height=800 ) ->

    @x0    = @width/2;   @y0 = @height/2
    @inner = 0;         @outer = ( Math.min(@width,@height) - 100 ) / 2
  ...
  doQuads:( quadrants ) =>
    @quads( Util.toArray(quadrants), @r08, @r100 )
    @circles( @criterias )
    return
  ...
  updateModel:( tech ) ->
    return unless Util.hasGlobal('backbone',false)
    id    = @collection.toId( tech.name )
    model = @collection.get( id )
    if model?
       model.set( { grade:tech.grade, angle:tech.angle } )
    else
