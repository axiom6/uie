...
class Store
  ...
  module.exports  = Util.Export( Store, 'store/Store'   )
  ...
  @readFile:( filePath, doFile ) ->
    fileReader = new FileReader()
    fileReader.onerror = (e) -> Util.error( 'Store.readFile', e.target.error )
    fileReader.onload  = ()  -> doFile( fileReader.results )
    fileReader.readAsText( filePath )
    return

  @readJson:( filePath, doJson ) ->
    fileReader = new FileReader()
    fileReader.onerror = (e) -> Util.error( 'Store.readFile', e.target.error )
    fileReader.onload  = (e) ->
    #console.log('Store.readJson', e.target.result )
      doJson( JSON.parse( e.target.result) )
    fileReader.readAsText( filePath )
    return

  # A quick in and out method to retrieve JSON data
  @getJson:( stream, uri, table, doData ) ->
  ...
  # A quick in and out method to retrieve JSON data
  @getAll:( stream, uri, doData ) ->
    rest = new @Rest( stream, uri )
    rest.remember()
    rest.select( uri )
    rest.subscribe( Util.firstTok(uri,'.'), 'none', 'select', doData )

  @persistRest:( database ) ->
    Rest     = Util.Import( 'store/Rest' )
...
# params=Store provides empty defaults
  ...
  getMemory:() ->
    @hasMemory = true
    Store.memories[@dbName] = new Store.Memory( @stream, @dbName ) if not Store.memories[@dbName]?
...
# ops can be single value. ids can be an array for single record ops
  ...
  onError2:( error ) ->
    Util.error( 'Store.onError()', error.params, error.result )

  onComplete:() ->
    Util.log(   'Store.onComplete()', 'Completed' )
  ...
  toObjectsJson:( json, where ) ->
    Util.toObjects( JSON.parse(json), where, @key )

  toKeysJson:( json ) ->
    @toKeys( JSON.parse(json) )
